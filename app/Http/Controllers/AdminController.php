...
class AdminController extends Controller
{
    ...
    {
        $products = DB::table('sanpham')->join('sub_menu', 'sanpham.id_sub', '=', 'sub_menu.id_sub')->select('sanpham.*', 'sub_menu.name_sub as category')->get();
    ...
    public function product_view(Request $request)
    {
        ...
        return view('admin.product_view', ['product' => $product[0], 'category' => $category[0]->name_sub]);
    }
    ...
    public function product_create()
    {
        ...
        $action = "add";
        return view("admin.product_form", ['menu' => $menu, 'sub_menu' => $sub_menu, 'action' => $action]);
    }
    public function product_save($action, Request $request)
    {
        $request->validate([
            'id_catalog' => ['required', 'string', 'max:50'],
            'id_sub' => ['required', 'string', 'max:50'],
            'ten_sp' => ['required', 'string', 'max:200'],
            'code_product' => ['required', 'string', 'max:50'],
            'price' => ['required', 'string', 'max:50'],
            'description' => ['required', 'string', 'max:300'],
            'content' => ['required', 'string', 'max:200'],
            'discount' => ['required', 'string', 'max:200'],
            'image_sp' => [],
            'xuatxu' => ['required', 'string', 'max:200'],
            'sizess' => ['required', 'string', 'max:200'],
            'mausac' => ['required', 'string', 'max:200'],
        ...
        return redirect()->route('product_view')->with('status', $message);
    }
